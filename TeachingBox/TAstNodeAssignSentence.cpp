...
TAstNode::ValueReturned TAstNodeAssignSentence::Execute() const
{
	...
	{
		TVariateManager::GetInstance()->UpdateInMap(var->GetScope(), var->GetName(), TInteger(var->GetScope(), var->GetName(), static_cast<int>(value.value)));
	...
	{
		TVariateManager::GetInstance()->UpdateInMap(var->GetScope(), var->GetName(), TBool(var->GetScope(), var->GetName(), value.value!=0));
