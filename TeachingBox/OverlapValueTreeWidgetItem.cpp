...
std::shared_ptr<TVariate> OverlapValueTreeWidgetItem::GetVariate(const TSymbol& symbol, QTreeWidget* treeWidget, QTreeWidgetItem* variateItem)
{
	auto modeName = dynamic_cast<QComboBox*>(treeWidget->itemWidget(variateItem->child(0), 1))->currentText();
	...
	auto value = dynamic_cast<QLineEdit*>(treeWidget->itemWidget(variateItem->child(1), 1))->text().toDouble();
